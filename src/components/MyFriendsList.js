...
export default class MyFriendsList extends Component { 
  state = { 
  }
  constructor(props) {
    super(props);
  ...
  fetchFriendsRequest(){
    ...
    fetch(apiUrl()+"get-my-friends?user_id="+user_id)
        .then(res => res.json())
        .then(
        (result) => {
            this.setState({
...
render () {     
      const { error, isLoaded,isLogin,friends,image_path,default_image,login_user_id } = this.state;
                        ...
                        <div class="jumbotron list-content">
                            <ul className={(friends.length <= 0)?'hide':''}>
                                ...
                                <li href="#" class="list-group-item text-left">
                                    ...
                                    <label class="name">
                                         {(frnd.user_id == login_user_id)?frnd.to_user_info.username:frnd.from_user_info.username} 
                                    </label>
